...
async function testLiveEndpoint() {
    try {
        ...
        // Create a test text that's over 2000 characters
        ...
        console.log(`Text length: ${testData.text.length} characters`);
        console.log('\nMaking request to live server...');

        // Make request to live server
        const response = await axios.post('https://your-live-server-url/api/tts', testData, {
...
// Ask for the live server URL before running the test
const readline = require('readline').createInterface({
    input: process.stdin,
    output: process.stdout
});

readline.question('Enter your live server URL: ', (url) => {
    readline.close();
    testLiveEndpoint(url);
});
